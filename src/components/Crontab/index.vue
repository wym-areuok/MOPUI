<template>
    <div>
        <el-tabs type="border-card">
            <el-tab-pane :label="$t('page.cron秒')" v-if="shouldHide('second')">
                <CrontabSecond
            ...
            <el-tab-pane :label="$t('page.cron分钟')" v-if="shouldHide('min')">
                <CrontabMin
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
            ...
            <el-tab-pane :label="$t('page.cron时')" v-if="shouldHide('hour')">
                <CrontabHour
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
            ...
            <el-tab-pane :label="$t('page.cron日')" v-if="shouldHide('day')">
                <CrontabDay
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
            ...
            <el-tab-pane :label="$t('page.cron月')" v-if="shouldHide('month')">
                <CrontabMonth
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
            ...
            <el-tab-pane :label="$t('page.cron周')" v-if="shouldHide('week')">
                <CrontabWeek
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
            ...
            <el-tab-pane :label="$t('page.cron年')" v-if="shouldHide('year')">
                <CrontabYear
                    @update="updateCrontabValue"
                    :check="checkNumber"
                    :cron="crontabValueObj"
        ...
        <div class="popup-main">
            <div class="popup-result">
                <p class="title">{{ $t('page.时间表达式') }}</p>
                <table>
                    <thead>
                        <tr>
                            <th v-for="item of tabTitles" :key="item">{{ item }}</th>
                            <th>{{ $t('page.cron 表达式') }}</th>
                        </tr>
                    </thead>
                    <tbody>
                        <tr>
            ...
            <div class="pop_btn">
                <el-button type="primary" @click="submitFill">{{ $t('page.确定') }}</el-button>
                <el-button type="warning" @click="clearCron">{{ $t('page.重置') }}</el-button>
                <el-button @click="hidePopup">{{ $t('page.取消') }}</el-button>
            </div>
        </div>
    </div>
</template>
...
const tabTitles = computed(() => [proxy.$t("page.cron秒"), proxy.$t("page.cron分钟"), proxy.$t("page.cron时"), proxy.$t("page.cron日"), proxy.$t("page.cron月"), proxy.$t("page.cron周"), proxy.$t("page.cron年")])
