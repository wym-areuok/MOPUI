<template>
  <div id="tags-view-container" class="tags-view-container" :class="{ 'tags-view-container--chrome': tagsViewStyle === 'chrome' }">
    ...
    <el-dropdown class="tags-action-dropdown" trigger="click" placement="bottom-end" @command="handleDropdownCommand">
      ...
      <template #dropdown>
        <el-dropdown-menu class="tags-dropdown-menu">
          <el-dropdown-item v-if="!isAffix(selectedDropdownTag)" command="close"><close style="width: 1em; height: 1em;" />{{ $t('page.关闭当前') }}</el-dropdown-item>
          <el-dropdown-item command="closeOthers"><circle-close style="width: 1em; height: 1em;" />{{ $t('page.关闭其他') }}</el-dropdown-item>
          <el-dropdown-item command="closeLeft" :disabled="isFirstView()"><back style="width: 1em; height: 1em;" />{{ $t('page.关闭左侧') }}</el-dropdown-item>
          <el-dropdown-item command="closeRight" :disabled="isLastView()"><right style="width: 1em; height: 1em;" />{{ $t('page.关闭右侧') }}</el-dropdown-item>
          <el-dropdown-item command="closeAll"><circle-close style="width: 1em; height: 1em;" />{{ $t('page.全部关闭') }}</el-dropdown-item>
          <el-dropdown-item command="fullscreen" divided>
            <template v-if="!isFullscreen"><full-screen style="width: 1em; height: 1em;" />{{ $t('page.全屏显示') }}</template>
            <template v-else><close style="width: 1em; height: 1em;" />{{ $t('page.退出全屏') }}</template>
          </el-dropdown-item>
        </el-dropdown-menu>
      </template>
    </el-dropdown>
    ...
    <!-- 刷新按钮 -->
    <span class="tags-action-btn tags-refresh-btn" :title="$t('page.刷新页面')" @click="refreshSelectedTag(selectedDropdownTag)">
      <el-icon><refresh-right/></el-icon> {{ $t('page.刷新') }}
    </span>

    <!-- 右键上下文菜单 -->
    <ul v-show="visible" :style="{ left: left + 'px', top: top + 'px' }" class="contextmenu">
      <li @click="refreshSelectedTag(selectedTag)"><refresh-right style="width: 1em; height: 1em;" />{{ $t('page.刷新页面') }}</li>
      <li v-if="!isAffix(selectedTag)" @click="closeSelectedTag(selectedTag)"><close style="width: 1em; height: 1em;" />{{ $t('page.关闭当前') }}</li>
      <li @click="closeOthersTags"><circle-close style="width: 1em; height: 1em;" />{{ $t('page.关闭其他') }}</li>
      <li v-if="!isFirstView()" @click="closeLeftTags"><back style="width: 1em; height: 1em;" />{{ $t('page.关闭左侧') }}</li>
      <li v-if="!isLastView()" @click="closeRightTags"><right style="width: 1em; height: 1em;" />{{ $t('page.关闭右侧') }}</li>
      <li @click="closeAllTags(selectedTag)"><circle-close style="width: 1em; height: 1em;" />{{ $t('page.全部关闭') }}</li>
    </ul>
  </div>
</template>
